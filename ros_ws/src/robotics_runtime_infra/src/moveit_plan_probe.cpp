...
int main(int argc, char** argv)
{
  ...
  try {
    ...
    if (!initial_state) {
      ...
      moveit::planning_interface::MoveGroupInterface::Plan rejected_plan;
      const auto rejected = move_group.plan(rejected_plan);
      if (rejected == moveit::core::MoveItErrorCode::SUCCESS) {
        RCLCPP_ERROR(node->get_logger(), "MoveIt planned an out-of-range target");
        status = 4;
